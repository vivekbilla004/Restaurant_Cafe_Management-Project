...
const InventoryManager = () => {
  ...
  const handleAddRecipe = async (e) => {
    ...
    try {
      ...
      toast.success("Recipe mapped successfully!", { id: loadToast });
      setRecipeForm({ inventoryId: "", requiredQty: "" });
      fetchData();
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to map recipe", {
        id: loadToast,
  ...
  // 🔥 NEW: Safe Delete Function
  const handleDeleteStock = async (id, name) => {
    if (
      !window.confirm(
        `⚠️ WARNING: Are you sure you want to delete "${name}"? This will also remove it from any linked menu recipes!`,
      )
    )
      return;

    const loadToast = toast.loading(`Deleting ${name}...`);
    try {
      await api.delete(`/api/inventory/${id}`);
      toast.success(`${name} deleted permanently.`, { id: loadToast });
      ...
      {activeTab === "stock" && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {inventory.map((item) => (
                <div key={item._id} className="p-4 bg-white relative">
                  ...
                  {item.isLowStock ? (
                    <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase text-red-600 bg-red-50 px-2 py-1 rounded-md border border-red-100">
                      <AlertCircle size={12} /> Low Stock
                    </span>
                  ) : (
                    <span className="inline-block text-[10px] font-black uppercase text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100">
                      Adequate
                    </span>
                  )}
