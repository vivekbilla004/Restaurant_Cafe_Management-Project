...
const TablesManager = () => {
  ...
  const handleReserve = async (e) => {
    ...
    try {
      ...
      // 🔥 THE ERROR REVEAL: This prints the EXACT backend failure to your browser Console (F12)
      console.error("FULL BACKEND ERROR:", err.response?.data);
      toast.error(
        err.response?.data?.message || "Server crashed during reservation",
      );
    }
  };
  ...
  const getTableStyle = (table) => {
    ...
    switch (table.status) {
      ...
      case "Occupied":
        return base + "bg-red-50 border-red-200 text-red-800 hover:bg-red-100";
      case "Reserved":
        return (
          base +
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Manage live table statuses, reservations, and merges.
          </p>
        </div>
            ...
            <>
              {isMergeMode ? (
                <>
                  ...
                  >
                    <CheckCircle size={18} /> Confirm Merge
                  </button>
                </>
              ) : (
                <button
                  onClick={() => setIsMergeMode(true)}
                  className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 shadow-sm"
                >
                  <LinkIcon size={18} /> Merge Tables
                </button>
              )}

              <button
                onClick={() => setIsAddModalOpen(true)}
                className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 shadow-sm"
              >
          ...
          >
            ...
            {table.status === "Reserved" && table.reservationName && (
              <div className="absolute inset-x-0 bottom-2 text-center px-1">
                <p className="text-xs font-bold truncate bg-amber-100 rounded text-amber-900">
        ...
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden p-6 animate-in fade-in zoom-in-95">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Current Status:{" "}
              <span className="font-bold uppercase">{activeTable.status}</span>
            </p>
            ...
            <div className="space-y-3 mb-6">
              {activeTable.status !== "Occupied" && (
                <button
                  ...
                  className="w-full py-3 bg-red-50 text-red-700 border border-red-200 font-bold rounded-xl hover:bg-red-100"
                >
              ...
              {activeTable.status !== "Available" && (
                <button
                  ...
                  className="w-full py-3 bg-green-50 text-green-700 border border-green-200 font-bold rounded-xl hover:bg-green-100"
                >
            ...
            {activeTable.status === "Available" && (
              <form onSubmit={handleReserve} className="border-t pt-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setReserveForm({ ...reserveForm, name: e.target.value })
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setReserveForm({ ...reserveForm, time: e.target.value })
                  }
                  className="w-full mb-3 px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                />
                <button
                  type="submit"
                  className="w-full py-2.5 bg-amber-500 text-white font-bold rounded-lg hover:bg-amber-600"
                >
                  Confirm Reservation
                </button>
              </form>
            ...
            <button
              ...
              className="w-full mt-4 py-2 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-lg"
            >
              Close
            </button>
          </div>
